...
export default function Alloy() {
  ...
  // 블러 - 설정 탭의 "테마" 카드에서 "시스템 설정" 바로 아래 체크박스로 켠다. 켜면
  // 폴더/이미지 갤러리 카드와 커버 선택 모달의 모든 썸네일에 강한 블러를 걸어(주변에
  // 화면이 보여도) 내용이 잘 안 보이게 한다 - 원본 파일 자체는 그대로다, 표시만 가린다.
  const [blurThumbnailsEnabled, setBlurThumbnailsEnabled] = useState(false);
  const toggleBlurThumbnails = () => setBlurThumbnailsEnabled((v) => !v);
  const thumbnailBlurStyle = blurThumbnailsEnabled ? { filter: "blur(24px)" } : undefined;
  ...
  const clearMyData = () => {
    ...
    setBlurThumbnailsEnabled(false);
    setGalleryViewPaths({});
    setCurrentPath([]);
  };
  ...
  const stickyHeaderStyle = {
    ...
    // 아이폰에서 홈 화면에 추가해 PWA(standalone)로 실행하면 viewport-fit=cover 때문에
    // 콘텐츠가 상태 표시줄 영역까지 올라간다 - 설정/휴지통 화면 헤더와 똑같이 안전 영역
    // 만큼 위쪽 여백을 더 준다(safe-area-inset-top이 0인 일반 브라우저에서는 기존 22px 그대로).
